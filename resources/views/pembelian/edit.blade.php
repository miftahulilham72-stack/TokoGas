...
<div class="card">
    <div class="card-body">
        <form action="{{ route('pembelian.update', $pembelian) }}" method="POST" id="formPembelian">
            @csrf
            @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('agen_id') is-invalid @enderror" 
                            ...
                            @foreach($agens as $agen)
                                <option value="{{ $agen->id }}" {{ old('agen_id', $pembelian->agen_id) == $agen->id ? 'selected' : '' }}>
                                    {{ $agen->nama }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control @error('tanggal_beli') is-invalid @enderror" 
                               id="tanggal_beli" name="tanggal_beli" value="{{ old('tanggal_beli', $pembelian->tanggal_beli->format('Y-m-d')) }}" required>
                        @error('tanggal_beli')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('no_faktur') is-invalid @enderror" 
                       id="no_faktur" name="no_faktur" value="{{ old('no_faktur', $pembelian->no_faktur) }}">
                @error('no_faktur')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div id="items-container">
                ...
                <div class="item-row row mb-2">
                    <div class="col-md-4">
                        <select class="form-select jenis-gas" name="items[{{ $index }}][jenis_gas_id]" required>
                            <option value="">Pilih Jenis Gas</option>
                            @foreach($jenisGases as $gas)
                                <option value="{{ $gas->id }}" {{ $detail->jenis_gas_id == $gas->id ? 'selected' : '' }}>
                                    {{ $gas->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-2">
                        <input type="number" class="form-control jumlah" name="items[{{ $index }}][jumlah]" 
                               value="{{ $detail->jumlah_beli }}" placeholder="Jumlah" min="1" required>
                    </div>
                    <div class="col-md-3">
                        <input type="number" class="form-control harga-beli" name="items[{{ $index }}][harga_beli]" 
                               value="{{ $detail->harga_beli_saat_itu }}" placeholder="Harga Beli" min="0" step="500" required>
                    </div>
                    <div class="col-md-2">
                        <input type="text" class="form-control subtotal" value="Rp {{ number_format($detail->jumlah_beli * $detail->harga_beli_saat_itu, 0, ',', '.') }}" readonly>
                    </div>
                    <div class="col-md-1">
                ...
                <div class="col-md-6 offset-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control form-control-lg fw-bold" 
                               id="total_harga_display" value="Rp {{ number_format($pembelian->total_harga, 0, ',', '.') }}" readonly>
                    </div>
                </div>
            </div>
